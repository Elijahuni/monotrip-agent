...
async def recommend_trip(
    ...
    preferences: str | None = Query(
        default=None, max_length=300, description="자유 텍스트 취향 (예: 맛집 위주)"
    ),
    travel_style: str | None = Query(
        default=None, max_length=50, description="여행 스타일 키 (예: 미식, 자연, 쇼핑)"
    ),
...
async def get_destination_guide(
    ...
    destination: str = Query(
        min_length=1, max_length=200, description="여행지 이름 (예: 도쿄, 파리)"
    ),
...
async def recommend_by_weather(
    ...
    weather_condition: str = Query(
        description=("날씨 조건 키. 가능한 값: " + ", ".join(WEATHER_CONDITION_LABELS.keys())),
