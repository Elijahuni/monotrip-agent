...
class CollaborationService:
    ...
    async def accept_invite(
        ...
        if invite.accepted_at is not None:
            raise HTTPException(status_code=status.HTTP_410_GONE, detail="이미 사용된 초대 링크입니다.")
        ...
        # 본인 여행은 협업자로 추가 불가
        if trip.user_id == user_id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="본인 여행에는 참여할 수 없어요.")
    ...
    async def list_collaborators(
        ...
        rows = await db.execute(
            select(TripCollaborator).where(TripCollaborator.trip_id == trip_id)
        )
        return list(rows.scalars().all())

    async def user_has_edit_access(
