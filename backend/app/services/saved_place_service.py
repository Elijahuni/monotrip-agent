...
class SavedPlaceService:
    ...
    async def save(self, db: AsyncSession, user_id: int, data: SavedPlaceCreate) -> SavedPlaceResponse:
    ...
    async def remove(self, db: AsyncSession, saved_place_id: int, user_id: int) -> None:
        ...
        if obj is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="저장된 장소를 찾을 수 없습니다.")
